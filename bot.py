...
# GIF untuk status searching & selesai (folder assets di sebelah bot.py)
ASSETS_DIR = Path(__file__).resolve().parent / "assets"
SEARCHING_GIF = ASSETS_DIR / "searching.gif"
DONE_GIF = ASSETS_DIR / "done.gif"
STATUS_DELETE_DELAY = 6  # detik sebelum pesan status dihapus
...
async def handle_prompt(
    ...
    try:
        ...
        if will_search:
            kwargs: dict[str, Any] = {
                "content": "🔍 **Pak Burhan sedang mencari** informasi di internet..."
            }
        ...
        async with target.typing():
            ...
            if status_msg is not None:
                try:
                    edit_kwargs: dict[str, Any] = {
                        "content": "📝 **Pak Burhan merangkum** hasil pencarian...",
                        "attachments": [],
                    }
                    if DONE_GIF.exists():
                        edit_kwargs["file"] = discord.File(DONE_GIF)
            ...
            if status_msg is not None:
                await asyncio.sleep(STATUS_DELETE_DELAY)
                try:

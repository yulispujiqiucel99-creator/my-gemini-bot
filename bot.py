"""Pak Burhan v2.9.3 - Discord bot wali kelas 7D.
...
class DuckDuckGoParser(HTMLParser):
    """Parser kecil untuk halaman HTML DuckDuckGo tanpa dependency tambahan."""
    ...
    def handle_starttag(self, tag: str, attrs: list[tuple[str, Optional[str]]]) -> None:
        ...
        if tag == "a" and "result__a" in classes and attributes.get("href"):
            if len(self.results) < MAX_SEARCH_RESULTS:
                self._current = {
                    "title": "",
                    "url": self._decode_url(attributes["href"] or ""),
                    "snippet": "",
                }
                self._mode = "title"
    ...
    def handle_endtag(self, tag: str) -> None:
        if tag == "a" and self._current and self._mode == "title":
            self.results.append(self._current)
            self._current = None
            self._mode = None
        elif self._current and self._mode == "snippet" and tag == "div":
            self._mode = None
...
def clean_search_query(prompt: str, max_words: int = 12) -> str:
    ...
    Query panjang berupa kalimat ngobrol biasa sering gak dapat match
    organik yang bagus di DuckDuckGo, dan malah nyasar ke hasil
    sponsor/iklan yang gak nyambung sama sekali. Jadi kata pemicu dan
    basa-basi dibuang dulu, sisanya dipotong ke beberapa kata kunci inti.
    """
...
def _search_web_sync(query: str) -> list[dict[str, str]]:
    ...
    request = Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 (compatible; PakBurhanBot/2.9)",
...
async def search_web(query: str) -> list[dict[str, str]]:
    try:
        return await asyncio.to_thread(_search_web_sync, query[:500])
...
async def handle_prompt(
    ...
    try:
        prepared_prompt = await prepare_ai_prompt(prompt, attachments or [])
...
async def ask(interaction: discord.Interaction, pertanyaan: str) -> None:
    try:
        ...
        prepared_prompt = await prepare_ai_prompt(pertanyaan, [])
...
async def searching(interaction: discord.Interaction, pertanyaan: str) -> None:
    try:
        ...
        # force_search=True: selalu cari web, gak nebak dari kata kunci.
        prepared_prompt = await prepare_ai_prompt(pertanyaan, [], force_search=True)
        author_name = getattr(interaction.user, "display_name", interaction.user.name)
        channel_id = interaction.channel_id or interaction.user.id
        answer = await ask_openrouter(
            interaction.user.id,
